...
// FromPBTx get tx
func FromPBTx(tx *xldgpb.Transaction) *Transaction {
	t := &Transaction{
		Txid:              tx.Txid,
		Blockid:           tx.Blockid,
		Nonce:             tx.Nonce,
...
func FromInternalBlockPB(block *xldgpb.InternalBlock) *InternalBlock {
...
func FromPBJustify(qc *xldgpb.QuorumCert) *QuorumCert {
	justify := &QuorumCert{}
	if qc != nil {
...
func FromChainStatusPB(statuspb *xchainpb.QueryChainStatusResp) *ChainStatus {
	ledgerMeta := statuspb.GetLedgerMeta()
	utxoMeta := statuspb.GetUtxoMeta()
	...
	status := &ChainStatus{
		...
		BranchBlockid: statuspb.GetBranchBlockId(),
	}
