...
func (t *QCPendingTree) UpdateQcStatus(node *ProposalNode) error {
	...
	t.log.Debug("QCPendingTree::updateQcStatus", "insert new", utils.F(node.In.GetProposalId()), "height", node.In.GetProposalView(), "highQC", utils.F(t.highQC.In.GetProposalId()))
